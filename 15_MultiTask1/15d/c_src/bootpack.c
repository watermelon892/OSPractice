  ...
  for (;;) {
    ...
    if (fifo32_status(&fifo) == 0) {
      ...
      if (i == 2) {
        ...
        putfonts8_asc_sht(sht_back, 0, 64,
                          COL8_FFFFFF, COL8_008484,
                          "10[sec]", 7);
      } else if (i == 3) {
        putfonts8_asc_sht(sht_back, 0, 80,
                          COL8_FFFFFF, COL8_008484,
                          "3[sec]", 6);
      } else if (i <= 1) {
        if (i != 0) {
...
void task_b_main(void) {
  ...
  timer_settime(timer_ts, 2);
  sht_back = (struct SHEET *) *((int *)0x0fec); // BOOTINFO - 4

  for (;;) {
    count++;
    my_sprintf(s, "%9d", count);
