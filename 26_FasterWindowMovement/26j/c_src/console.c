...
void cmd_start(struct CONSOLE *cons, char *cmdline, int memtotal) {
  ...
  /* Enter the strings entered on the command line
     character by character into the new console */
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline) {
  ...
  if (finfo != 0) {
    ...
    if (finfo->size >= 36 && my_strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
      ...
      start_app(0x1b, task->sel + 1000 * 8, esp,
                task->sel + 2000 * 8, &(task->tss.esp0));
  ...
  */
  ...
  if (edx == 1) {
    ...
    for (;;) {
      io_cli();
      if (fifo32_status(&task->fifo) == 0) {
        if (eax != 0) task_sleep(task); // FIFO is empty, sleep and wait
        else {
          io_sti();
          reg[7] = -1;
          return 0;
      ...
      if (i == 2) cons->cur_c = COL8_FFFFFF; // Turn on cursor
      if (i == 3) cons->cur_c = -1;          // Turn off cursor
      if (i >= 256) {
        /* Keyboard data. (Via task A) */
        reg[7] = i - 256;
        return 0;
      }
    }
...
void hrb_api_linewin(struct SHEET *sht, int x0, int y0,
