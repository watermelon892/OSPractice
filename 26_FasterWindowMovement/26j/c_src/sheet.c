/* Mouse and window overlay processing */
...
struct SHEET *sheet_alloc(struct SHTCTL *ctl) {
  ...
  for (i = 0; i < MAX_SHEETS; i++) {
    if (ctl->sheets0[i].flags == 0) {
      ...
      sht->height = -1;       // Hidden
      sht->task = 0;          // Don't use the automatic closing
      return sht;
    }
  }
  return 0;  // All seets are in use
}
...
void sheet_refreshsub(
  ...
  /* Correct if it extends outside the screen */
  if (vx0 < 0) vx0 = 0;
  if (vy0 < 0) vy0 = 0;
  if (vx1 > ctl->xsize) vx1 = ctl->xsize;
  if (vy1 > ctl->ysize) vy1 = ctl->ysize;
...
void sheet_updown(struct SHEET *sht, int height) {
  ...
  int h, old = sht->height; // Save the current height
