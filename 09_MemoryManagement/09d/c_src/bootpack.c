...
unsigned int memtest(unsigned int start, unsigned int end) {
  ...
  /* Check if it is 386 or 486 or later */
  eflg = io_load_eflags();
  eflg |= EFLAGS_AC_BIT;  // AC-bit = 1
  io_store_eflags(eflg);
  eflg = io_load_eflags();
  if ((eflg & EFLAGS_AC_BIT) != 0) {
    /* On 386, even if AC is set to 1, it automatically returns to 0. */
    flg486 = 1;
  }
  ...
  if (flg486 != 0) {
    cr0 = load_cr0();
    cr0 |= CR0_CACHE_DISABLE; // Temporarily disable the cache function
    store_cr0(cr0);
  }
  ...
  if (flg486 != 0) {
    cr0 = load_cr0();
    cr0 &= ~CR0_CACHE_DISABLE; // Enable the cache function
    store_cr0(cr0);
  }
...
void memman_init(struct MEMMAN *man) {
  man->frees    = 0;  // Number of free memory information
  man->maxfrees = 0;  // For situation observation : Maximum value of frees
  man->lostsize = 0;  // Total size that failed to free
  man->losts    = 0;  // Number of times failed to free
}
